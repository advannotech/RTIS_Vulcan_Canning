...
namespace RTIS_Vulcan_Canning.Controls
{
    public partial class ucSettings : UserControl
    {
        ...
        public ucSettings(Panel _parent, Form1 _main)
        {
            InitializeComponent();
            parent = _parent;
            main = _main;
        ...
        public void getSettings()
        {
            try
            {
                ...
                txtAWWhse.Text = GlobalVars.canningWhse;
                txtAWIT.Text = GlobalVars.canningIT;
                txtZectLabel.Text = GlobalVars.canningLoc;
                cmbPrinter.Text = GlobalVars.Printer;
                txtLotLook.Text = GlobalVars.LotLookup;
            }
            catch (Exception ex)
            {
                ExHandler.showErrorEx(ex);
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtIP.Text != string.Empty && txtIP2.Text != string.Empty && txtIP3.Text != string.Empty && txtIP4.Text != string.Empty)
                {
                    ...
                    {
                        if (txtAWWhse.Text != string.Empty && txtAWIT.Text != string.Empty)
                        {
                            if (txtZectLabel.Text != string.Empty)
                            {
                                ...
                                updateCommand += "UPDATE [Settings] SET [Value] ='" + txtAWWhse.Text + "' WHERE [SettingName] = 'CanningWhse';";
                                updateCommand += "UPDATE [Settings] SET [Value] ='" + txtAWIT.Text + "' WHERE [SettingName] = 'CanningIT';";
                                updateCommand += "UPDATE [Settings] SET [Value] ='" + txtZectLabel.Text + "' WHERE [SettingName] = 'CanningPath';";
                                updateCommand += "UPDATE [Settings] SET [Value] ='" + cmbPrinter.Text + "' WHERE [SettingName] = 'Printer';";
                                updateCommand += "UPDATE [Settings] SET [Value] ='" + txtLotLook.Text + "' WHERE [SettingName] = 'LotLookup';";
                                ...
                                switch (updated.Split('*')[0])
                                {
                                    case "1":
                                        ...
                                        GlobalVars.canningWhse = txtAWWhse.Text;
                                        GlobalVars.canningIT = txtAWIT.Text;
                                        GlobalVars.canningLoc = txtZectLabel.Text;
                                        GlobalVars.Printer = cmbPrinter.Text;
                                        GlobalVars.LotLookup = txtLotLook.Text;
                                        break;
                                    case "-1":
        ...
        private void txtAWWhse_Click(object sender, EventArgs e)
        {
            GlobalVars.focusedEdit = txtAWWhse;
        }

        private void txtAWIT_Click(object sender, EventArgs e)
        {
            GlobalVars.focusedEdit = txtAWIT;
        }
        ...
        private void btnCanningPath_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        private void txtLotLook_Click(object sender, EventArgs e)
        {
            GlobalVars.focusedEdit = txtLotLook;
        }
    }
}
